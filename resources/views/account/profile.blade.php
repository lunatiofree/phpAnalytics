...
<div class="card border-0 shadow-sm @if(request()->is('admin/*')) mb-3 @endif">
    ...
    <div class="card-body">
        ...
        <form action="{{ (request()->is('admin/*') ? route('admin.users.edit', $user->id) : route('account.profile.update')) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="i-avatar">{{ __('Avatar') }}</label>
                <div class="input-group">
                    <div class="input-group-prepend">
                        <span class="input-group-text py-1 px-2"><img src="{{ $user->avatarUrl }}" style="max-height: 1.625rem"></span>
                    </div>
                    @if ($user->avatar)
                        <div class="btn-group-toggle input-group-prepend" data-toggle="buttons">
                            <label class="btn btn-outline-danger">
                                <input type="checkbox" name="remove_avatar" value="1"> {{ __('Remove') }}
                            </label>
                        </div>
                    @endif
