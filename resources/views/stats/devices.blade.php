                                    ...
                                    <div class="d-flex align-items-center text-truncate">
                                        @if(isset($first->value))
                                            <div class="d-flex align-items-center {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}">
                                                <img src="{{ asset('img/icons/devices/'.formatDevice($first->value ?? '')) }}.svg" class="width-4 height-4">
                                            </div>

                                            <div class="flex-grow-1 font-weight-bold text-truncate {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}">
                                                @if($first->value)
                                                    <span data-tooltip="true" title="{{ __(Str::ucfirst($first->value)) }}">{{ Str::ucfirst($first->value) }}</span>
                                                @else
                                                    {{ __('Unknown') }}
                                                @endif
                                            </div>
                            ...
                            <div class="row">
                                <div class="col">
                                    <div class="d-flex align-items-center text-truncate">
                                        @if(isset($last->value))
                                            <div class="d-flex align-items-center {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}">
                                                <img src="{{ asset('img/icons/devices/'.formatDevice($last->value ?? '')) }}.svg" class="width-4 height-4">
                                            </div>

                                            <div class="flex-grow-1 font-weight-bold text-truncate {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}">
                                                @if($last->value)
                                                    <span data-tooltip="true" title="{{ __(Str::ucfirst($last->value)) }}">{{ Str::ucfirst($last->value) }}</span>
                                                @else
                                                    {{ __('Unknown') }}
                                                @endif
                                            </div>
                        ...
                        <div class="list-group-item px-0 border-0">
                            <div class="d-flex flex-column">
                                ...
                                <div class="progress height-1.25 w-100">
                                    <div class="progress-bar bg-primary rounded" role="progressbar" style="width: {{ (($device->count / $total->count) * 100) }}%"></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
