...
trait UserTrait
{
    ...
    protected function userUpdate(Request $request, User $user)
    {
        ...
        if ($request->has('remove_avatar')) {
            ...
            try {
                Image::load($request->file('avatar')->getRealPath())->fit(Manipulations::FIT_CROP, config('settings.user_avatar_size'), config('settings.user_avatar_size'))->format(Manipulations::FORMAT_JPG)->quality(80)->save();
