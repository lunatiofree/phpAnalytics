...
trait DateRangeTrait
{
     ...
     * @return array
     */
    private function range($from = null, $to = null, bool $dynamic = true)
    {
        $now = Carbon::now()->startOfDay();

        try {
            $to = request()->input('to') && $dynamic ? Carbon::createFromFormat('Y-m-d', request()->input('to')) : ($to ? $to->startOfDay() : $now);
        } catch (\Exception $e) {
            $to = $now;
        }

        try {
            $from = request()->input('from') && $dynamic ? Carbon::createFromFormat('Y-m-d', request()->input('from')) : ($from ? $from->startOfDay() : $to);
